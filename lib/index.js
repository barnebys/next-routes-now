...
const build = (module.exports.build = async (routesPath, nowConfigPath) => {
  ...
  const routes = routesConfig.reduce((acc, route) => {
    if (route.build) {
      ...
      const routes = data.routes.map(nextRoute => {
        const query =
          nextRoute.keyNames.length > 0
            ? "?" +
              nextRoute.keyNames.map((name, i) => `${name}=$${i + 1}`).join("&")
            : "";
        return {
          src: nextRoute.regex.source,
          dest: (route.prefix || "") + nextRoute.page + query
        };
      });
